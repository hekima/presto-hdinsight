...
if [[ $(hostname -s) = hn0-* ]]; then 
  ...
  cd /var/lib/presto
  wget https://github.com/hekima/presto-hdinsight/archive/master.tar.gz -O presto-hdinsight.tar.gz
...
#!/bin/bash
presto-cli --server $(slider registry  --name presto1 --getexp presto |  grep value | grep -o '[^"]*:[0-9]\+') --catalog hive "\$@"
EOF
